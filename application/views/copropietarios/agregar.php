...
<?php echo form_open(NULL, array("class" => "form-horizontal" , "autocomplete"=>"off"))?>
    ...
    <div class="form-group">
        <label class="col-md-3 control-label"> <i class="fa fa-asterisk" aria-hidden="true"></i> UNIDAD(ES)</label>
        <div class="col-md-9">
            <select name="unidad[]" class="form-control selectpicker" multiple data-live-search="true" data-selected-text-format="count">
                <optgroup label="UNIDADES"><?<?php 
                foreach($unidades as $value){
                    ?>
                    <option value="<?php echo $value['id_unidad']?>" data-subtext="/  <?php echo $value['tipo_unidad']?> / PISO <?php echo $value['piso']?>" <?php echo set_select('unidad',$value['id_unidad'])?> ><?php echo $value['nombre']?></option><?php
                }?>
                </optgroup>
            </select>
        </div>
